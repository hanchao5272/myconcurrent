...
public class SemaphoreBasicDemo {
    ...
    private static void otherAcquireMethod(Semaphore semaphore) throws InterruptedException {
        //acquierUninterruptibly():不可中断地获取1个许可证
        //---此线程会一直阻塞，直到获取这个许可证；中断无效。
        semaphore.acquireUninterruptibly();

        //acquireUninterruptibly(permits):不可中断地获取n个许可证
        //此线程会一直阻塞，直到获取全部n个许可证；中断无效。
        semaphore.acquireUninterruptibly(2);

        //tryAcquire():尝试获取1个许可证
        //---此线程在这个方法被调用时，进行一次获取1个许可证的尝试。
        //---如果获取到这个许可证，则返回true；反之返回false，并不再等待。
        semaphore.tryAcquire();

        //tryAcquire(permits):尝试获取n个许可证
        //---此线程在这个方法被调用时，进行一次获n个许可证的尝试。
        //---如果获取到全部n个许可证，则返回true；反之返回false，并不再等待。
        semaphore.tryAcquire();

        //tryAcquire(timeout,TimeUnit):限时尝试获取1个许可证
        //---此线程在这个方法被调用时，会在限定时间内去尝试获取1个许可证。
        //---如果获取到这个许可证，则返回true；如果限时超时，则返回false；如果被中断，则抛出InterruptedException异常。
        semaphore.tryAcquire(1, TimeUnit.SECONDS);

        //tryAcquire(permits,timeout,TimeUnit):限时尝试获取n个许可证
        //---此线程在这个方法被调用时，会在限定时间内去尝试获取n个许可证。
        //---如果获取到这n个许可证，则返回true；如果限时超时，则返回false；如果被中断，则抛出InterruptedException异常。
